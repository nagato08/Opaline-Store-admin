...
export default async function CampaignsPage({ searchParams }: PageProps<'/campagnes'>) {
  ...
  const now = new Date();
  const all = await listCampaigns(session);
  const filtered = state ? all.filter((campaign) => campaign.state === state) : all;
  ...
  const tabs = [
    ...
    ...(['running', 'scheduled', 'ended'] as const).map((key) => ({
      ...
      count: all.filter((campaign) => campaign.state === key).length,
    })),
  ];
            ...
            {filtered.map((campaign) => (
              <Row key={campaign.id}>
                <Cell>
                  ...
                  <span className="mt-0.5 block text-xs text-ink-500">{campaign.audience}</span>
                </Cell>
                <Cell>
                ...
                <Cell>
                  <Badge tone={SCHEDULE_STATES[campaign.state].tone}>
                    <Dot />
                    {SCHEDULE_STATES[campaign.state].label}
                  </Badge>
                </Cell>
